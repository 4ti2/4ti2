...
const int SupportTree<IndexSet>::INDENT = 2;
...
template <class IndexSet>
bool
SupportTree<IndexSet>::dominated(const SupportTreeNode& node, const IndexSet& b) const
{
    if (node.index < 0)
    {
...
template <class IndexSet>
bool
SupportTree<IndexSet>::dominated(const SupportTreeNode& node, const IndexSet& b, int index1, int index2) const
{
    if (node.index < 0)
    {
        for (unsigned int i = 0; i < node.nodes.size(); ++i)
...
template <class IndexSet>
int
SupportTree<IndexSet>::index_dominated(const SupportTreeNode& node, const IndexSet& b, int index1, int index2) const
{
    if (node.index < 0)
    {
        int index;
...
template <class IndexSet>
int
SupportTree<IndexSet>::find(const SupportTreeNode& node, const IndexSet& support, int start, int remaining) const
{
    if (remaining > 0)
    {
        // There should always be another unless remaining == 0.
...
SupportTree<IndexSet>::find_diff(
                const SupportTreeNode& node, std::vector<int>& indices,
...
template <class IndexSet>
void
SupportTree<IndexSet>::dump(const SupportTreeNode& node, int level) const
{
