...
main()
{
    ...
    _4ti2_state* rays_api = _4ti2_rays_create_state(_4ti2_PREC_INT_64);
    const int argc = 2;
    char*argv[2] = { (char*) "rays", (char*) "-q" };
    _4ti2_state_set_options(rays_api, argc, argv);

    _4ti2_matrix* cons_matrix;
    _4ti2_state_create_matrix(rays_api, m, n, "mat", &cons_matrix);
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
    ...
    _4ti2_state_create_matrix(rays_api, 1, m, "rel", &rel_matrix);
    for (int i = 0; i < m; ++i) {
    ...
    _4ti2_state_create_matrix(rays_api, 1, n, "sign", &sign_matrix);
    for (int i = 0; i < n; ++i) {
        _4ti2_matrix_set_entry_int64_t(sign_matrix, 0, i, sign[i]);
    }
    //_4ti2_matrix_write_to_stdout(sign_matrix);

    _4ti2_state_compute(rays_api);

    _4ti2_matrix* ray_matrix;
    _4ti2_state_get_matrix(rays_api, "ray", &ray_matrix);
    //_4ti2_matrix_write_to_stdout(ray_matrix);

    // Check the output
    if (_4ti2_matrix_get_num_rows(ray_matrix) != k) { return 1; }
    if (_4ti2_matrix_get_num_cols(ray_matrix) != n) { return 1; }
    for (int i = 0; i < k; ++i) {
        for (int j = 0; j < n; ++j) {
            _4ti2_int64_t value;
            _4ti2_matrix_get_entry_int64_t(ray_matrix, i, j, &value);
            if (value != ray[i][j]) { return 1; }
        }
    }
